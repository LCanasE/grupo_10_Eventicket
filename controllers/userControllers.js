...
const userControllers = {
  ...
  putEditUser: async (req, res) => {
    ...
    if (isCorrect) {
      ...
      newData.notificaciones === "on"
        ? (newData.notificaciones = true)
        : (newData.notificaciones = false);
      newData.tipoUsuario === "Espectador/a"
        ? (newData.tipoUsuario = 1)
        : (newData.tipoUsuario = 2);
    } else {
  ...
  postRegister: async (req, res) => {
    ...
    if (validation.errors.length > 0) {
      ...
      if (!check) {
        ...
        newUser.notificaciones === "on"
          ? (newUser.notificaciones = true)
          : (newUser.notificaciones = false);
        newUser.tyc === "on" ? (newUser.tyc = true) : (newUser.tyc = false);
        newUser.tipoUsuario === "Espectador/a"
  ...
  postLogin: async (req, res) => {
    ...
    if (!searchedUser) {
      return res.redirect(
        "/users/login?error=El email o la contraseña es inválido"
      );
    }
    ...
    if (isCorrect) {
      ...
      return res.redirect(
        "/users/login?error=El email o la contraseña es invalido"
      );
    }
  },
